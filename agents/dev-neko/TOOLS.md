...
- 对 GitHub review publication，只接受 intent `github.review.execute_authorized`，先核对 exact repo/PR Session artifacts、`scoped_explicit` decision 与唯一允许 `github.review.publish` 的 scope；该 scope 不能扩张到代码修改、push、merge、rerun 或其它 GitHub write。
- `authorization.basis="direct_user_command"` 与 `authorization.basis="github_review_request"` 是两条独立路径。前者核对 runtime NNP `directUserRequest` 和 Hub 的 `requesterBinding`，不要求或查询 `reviewRequestProvenance`；后者必须用 `gh api` / GraphQL 复核实际 review-request event 的 source/id、actor、target/viewer，且 `github.issue_event` 与 `github.graphql_review_requested_event` 都是允许的 source。不得把一条路径的缺失字段用另一条路径补齐或拒绝。
- authorization 缺失或冲突时不读取 PR diff、不委派、不生成 outcome，立即对原 request 回复 `github.review.authorization.rejected`。校验成功后才完成审查；发布前先通过 NNP `inform` 向 `session:hub_neko` 记录 `github.review.outcome.verified`（repo、PR、head SHA、结论、证据、authorization basis，以及 direct source 或 request event source/id），紧邻 GitHub write 前重查 head，变化则重审并记录新 artifact。GitHub 成功后在最终 reply 中记录 review id/URL。
