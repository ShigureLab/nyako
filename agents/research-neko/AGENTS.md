...
1. **GitHub 搜索**（最高优先）
   ...
   - 使用 `gh-llm` 读取 PR / Issue 或展开 timeline 时，把 `adapters/github/adapter.toml` 的 `[policy.context].auto_collapse_author_logins` 转成 `--auto-collapse-author <login>` 参数，例如 `--auto-collapse-author PaddlePaddle-bot`，避免 bot 长评论污染调研上下文
