...
- 读取 PR / Issue 完整上下文或展开 timeline 时，必须把 `adapters/github/adapter.toml` 的 `[policy.context].auto_collapse_author_logins` 转成 `gh-llm` 参数，例如 `--auto-collapse-author PaddlePaddle-bot`。该参数适用于 `pr view`、`pr timeline-expand`、`issue view`、`issue timeline-expand`，用于折叠噪声作者内容，避免 bot 长评论干扰判断。
...
- 调 `github_monitor_ledger` 时，GitHub inbox 通知的 `eventKey` 必须是 `github:thread:<thread_id>`，Session PR 状态反查事件使用 `github:session-pr:<session_id>:<repo>#<pr>` 这类稳定 key；不要发明 `gh-thread:*` / `github-notification:*` 等别名。优先传结构化 `state`，字段只包含可行动事实：`repo`、`pr` / `issue`、`headSha`、`state` / `terminal`、`merged` / `closed`、`reviewDecision`、`latestReviewId`、`latestReviewRequestId`、`latestCommentId`、`failedChecks`、`gate`。如果 PR 已 `merged` / `closed` 但触发点是新的可行动 review/comment，仍必须把 `latestReviewId` / `latestCommentId` 放进同一个 terminal state；不要只传 merged/closed digest。不要手写 `stateDigest`，除非工具环境暂时不支持 `state`；fallback `stateDigest` 也不能包含时间戳、轮询次数、临时 in-progress 细节、已失败检查数量这类会导致重复上报的噪声。
- 对 `reason=review_requested`，先用 `gh api user --jq .login` 取得当前执行账号，再用 `gh api repos/<owner>/<repo>/issues/<pr>/events --paginate` 或 GraphQL `ReviewRequestedEvent` timeline 找到实际直接请求该账号的事件。把事件 actor 传为 ledger `actorLogin`、事件 target 传为 `requestedReviewerLogin`、事件 id 传为 `state.latestReviewRequestId`；在 NNP payload 原样保留 event source/id、actor、target、`gh api user` 返回的 viewerLogin、requestedAt 和核验结果。通知 reason、PR author 或显示名都不能替代这些字段。
- `github_monitor_ledger` 会从 `adapters/github/adapter.toml` 的 `[policy.monitor].ignored_actor_logins` 读取 ignored actor。确认通知 / review / comment / check-run 的 `actorLogin` 后，用该 actorLogin 做 `action="check"`；若返回 `isIgnoredActor=true`，不要发送 NNP，也不要继续深挖，只 DELETE 对应 inbox thread 标记 done。
...
- 对已有业务 Session 的 session PR backcheck，同一 head 的 CI 状态默认由 monitor 内部消化；failed check 集合增删、顺序变化、workflow 展示名变化或 approval/check 上下文分拆，只有在完整 GitHub 上下文证明存在新的可行动根因时才上报。已验证重复、已暂停跟进、无新动作、stale goal 文本这类状态只 `record outcome="suppressed"`，不要发给中枢喵或平台 channel 触发 duplicate ack。
...
- 对已经成功上报中枢喵或明确决定忽略的事件，要立刻用 `action="record"` 记录 `outcome="routed"` 或 `outcome="suppressed"`；发送失败、工具不可用、`hub_neko` 未确认、或无法确认交付时不要记录。
...
- Session 路由判断先查 runtime session tools，但只用于确认 `hub_neko` 和生成建议目标；不要手工假设某个业务 Session 活跃。
