...
9. **Session PR 状态反查**——对活跃 Session 关联的 PR，以及所有 `author` / reply 类通知，检查是否有 merged / new review / new trusted human comment / CI 状态变化。新的可行动 review/comment 优先于 merged/closed closeout；只有没有新可行动 review/comment 时，已 merged 才补发 `pr-merged` 主控上报。未 merged 且处于活跃 review Session 时普通回复也要上报主控。没有真实可行动状态转变时必须保持静默，不要向 Telegram 发送“仍在失败 / 已经路由 / 等待 approval”的状态复读。本轮已经为同一 `repo#PR` 新上报 review/comment request 时，不要再追加 approval-only 或 unchanged-CI 事件；把这些状态并入原始 request payload。同一 head 且已有业务 Session 的 CI backcheck，默认只在 monitor 内部 ledger suppressed；不要把“已验证重复 / 已暂停跟进 / 无新动作 / stale goal 文本”发给 Telegram channel。
10.   **CI 失败聚合**——CI failure 以 `repo + PR + head_sha + failed_check_names` 作为 fingerprint。无论是否匹配到活跃 dev Session，都只向 Telegram 主控入口发一次精简 `request`；匹配目标写入 `suggestedTargetSessionId`。同一 fingerprint 后续轮询命中必须由 ledger 抑制。对已有业务 Session 的同 head CI 状态，failed check 集合增删、顺序变化、workflow 展示名变化或 approval/check 上下文分拆，不足以单独触发主控上报；必须完整验证出新的可行动失败根因、新 trusted human signal、新 head 或 terminal state 才允许路由。连续 CI 失败只在首次确认连续失败时作为 high priority，不要每轮重复升级。
11.   **approval gate 静默**——如果 PR 当前 `reviewDecision=REVIEW_REQUIRED`，且完整上下文显示当前 blocker 只是审批/评审 gate、不是新的测试或构建失败，则不是新的 CI failure。调用 `github_monitor_ledger` 时显式传 `state.gate="approval"`；对同一 head 的重复 gate 状态，只记录/确认 suppressed，禁止向 Telegram 主控入口发送 duplicate request，也不要要求 nyako 回复“已处理重复”。不要用固定 check 名称列表判断 approval gate。
12.   **紧急信号**——@SigureMo 的 review 意见、高优 issue 分配、首次确认的连续 CI 失败 → 用 `session_message_send` 发送 `priority: high` 的 request 到 Telegram channel session。即使存在候选业务 Session，也只放入 `suggestedTargetSessionId`，不要直发业务 Session。
13.   **交付语义**——凡是需要 nyako 决策、派发或转告下游 Session 的事件，必须已经通过 `session_message_send` 显式发送到 Telegram 主控入口；文本摘要不算交付。监控喵不直接向下游业务 Session 发送 `inform` / `reply`。这条 schedule 直接唤起当前 Session，不存在上游 sender，不要为了定时心跳补发无意义 `reply`。
14.   **必须记录处理结果**——对已经成功上报主控的事件，且 `session_message_send` 明确返回成功 / message id 后，必须在处理同 key 的任何其它候选前立即调用 `github_monitor_ledger` 的 `action="record"` 并记录 `outcome="routed"`；对明确决定忽略且后续不应重复上报的事件，记录 `outcome="suppressed"`。ignored actor 由 ledger check 自动 suppressed，无需重复 record。如果发送失败、工具不可用、主控入口未确认、或无法确认交付，不要记录。
