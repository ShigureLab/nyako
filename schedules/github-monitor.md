...
9. **Session PR 状态反查**——对活跃 Session 关联的 PR，以及所有 `author` / reply 类通知，检查是否有 merged / new review / new trusted human comment / CI 状态变化。新的可行动 review/comment 优先于 merged/closed closeout；只有没有新可行动 review/comment 时，已 merged 才补发 `pr-merged inform`。未 merged 且处于活跃 review Session 时普通回复也要补路由。没有真实可行动状态转变时必须保持静默，不要向 Telegram 发送“仍在失败 / 已经路由 / 等待 approval”的状态复读。本轮已经为同一 `repo#PR` 新路由 review/comment request 时，不要再向该 Session 追加 approval-only 或 unchanged-CI `inform`；把这些状态并入原始 request payload。
