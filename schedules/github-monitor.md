...
12.   **紧急信号**——@SigureMo 的 review 意见、高优 issue 分配、首次确认的连续 CI 失败 → 用 `nnp_send(toPeerId="session:hub_neko", kind="inform", ...)` 发送，并把 `priority: "high"` 放入 payload。即使存在候选业务 Session，也只放入 `suggestedTargetSessionId`，不要直发业务 Session。
13.   **交付语义**——凡是需要中枢喵决策、派发或转告下游 Session 的事件，必须已经通过 `nnp_send` 显式发送到 `session:hub_neko`；文本摘要不算交付。监控喵不直接向下游业务 Session 发送消息。这条 schedule 直接唤起当前 Session，不存在上游 sender，不要为了定时心跳补发无意义 `reply`。
14.   **必须记录处理结果**——对已经成功上报中枢喵的事件，且 `nnp_send` 明确返回成功 / message id 后，必须在处理同 key 的任何其它候选前立即调用 `github_monitor_ledger` 的 `action="record"` 并记录 `outcome="routed"`；对明确决定忽略且后续不应重复上报的事件，记录 `outcome="suppressed"`。ignored actor 由 ledger check 自动 suppressed，无需重复 record。如果发送失败、工具不可用、`hub_neko` 未确认、或无法确认交付，不要记录。
