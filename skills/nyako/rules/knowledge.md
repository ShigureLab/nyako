...
- **整理归纳 (Organize)**:
  ...
  - 必须实际执行文件移动或重命名，不可只描述而不落盘。
  - 建立或更新索引文件（如 `README.md` 或 `INDEX.md`）以便检索。
- **提炼知识 (Refine)**:
...
**注意：** 当用户要求“复习”或“整理归纳”时，除了总结内容与更新元数据，**必须执行整理归纳的实际操作**，包括但不限于：

1. 按主题/仓库/模块重新归类并移动或重命名文件。
2. 更新或新建索引文件，反映新的树状结构。
3. 清理空目录，确保层级清爽。
4. 在回复中列出实际变更的文件路径与归类结果。
